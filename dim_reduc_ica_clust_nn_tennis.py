...
np.savetxt('ica_sil_em_tennis', sil_em)

if False:
    for j in range(len(tit_cols)-1):
        plt.plot(ks, sil_em[j], linestyle='-', marker='o', label=str(j+1))
    plt.title("ICA EM silhouette curves - Tennis")
    plt.xlabel("number of clusters")
    plt.ylabel("silhouette score")
    plt.savefig("ica_em_sil_tennis.png")
    plt.legend(loc="best")
    plt.clf()
...
if False:
    ...
    np.savetxt('ica_nn_scores_tennis', ica_nn_scores)
    np.savetxt('ica_em_nn_scores_tennis', ica_em_nn_scores)
    np.savetxt('ica_km_nn_scores_tennis', ica_km_nn_scores)

    plt.plot(range(1, len(tit_cols)), ica_nn_scores, linestyle='-', marker='o')
    plt.title("ICA nn scores - Tennis")
    plt.xlabel("number of components")
    plt.ylabel("accuracy")
    plt.legend()
    plt.savefig("ica_nn_tennis.png")
    plt.clf()

    for j in range(len(tit_cols)-1):
        plt.plot(ks, ica_km_nn_scores[j], linestyle='-', marker='o', label=str(j+1))
    plt.title("ICA kmeans nn scores - Tennis")
    plt.xlabel("number of clusters")
    plt.ylabel("accuracy")
    plt.legend()
    plt.savefig("ica_km_nn_tennis.png")
    plt.clf()

    for j in range(len(tit_cols)-1):
        plt.plot(ks, ica_em_nn_scores[j], linestyle='-', marker='o', label=str(j+1))
    plt.title("ICA EM nn scores - Tennis")
    plt.xlabel("number of clusters")
    plt.ylabel("accuracy")
    plt.legend()
    plt.savefig("ica_em_nn_tennis.png")
    plt.clf()
